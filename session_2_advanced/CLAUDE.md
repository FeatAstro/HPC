# session_2_advanced — Hybrid-Kinetic PIC (Boris Pusher + Particle-Mesh Coupling + Field Solver)
...
This is the extension of `../session_2/`, which holds only what the session asks for (Steps 1-2,
prescribed fields). Here the fields are solved self-consistently (Step 3 below). The two folders
are independent CMake projects; a change to Steps 1-2 in one is not reflected in the other.
